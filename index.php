...
if(file_exists(__DIR__ . "/configure.json")){
	...
	if(isset($_POST["host"], $_POST["port"], $_POST["username"], $_POST["password"])){
		...
		$conn = mysqli_connect($config["host"] . ":" . $config["port"], $config["username"], $config["password"], "xss_lab");
...
SQL);
			...
			mysqli_query($conn, <<<SQL
			INSERT INTO users (name, email, password, picture, description) VALUES ("user1", "user1@email.com", "$pass", "user1.jpg", TO_BASE64('description of user 1'));
SQL);
		
			$pass = hash("md5", $password[1]);
			mysqli_query($conn, <<<SQL
			INSERT INTO users (name, email, password, picture, description) VALUES ("user2", "user2@email.com", "$pass", "user2.jpg", TO_BASE64('description of user 2'));
SQL);

			$pass = hash("md5", $password[2]);
			mysqli_query($conn, <<<SQL
			INSERT INTO users (name, email, password, picture, description) VALUES ("user3", "user3@email.com", "$pass", "user3.jpg", TO_BASE64('description of user 3'));
SQL);

			$pass = hash("md5", $password[3]);
			mysqli_query($conn, <<<SQL
			INSERT INTO users (name, email, password, picture, description) VALUES ("user4", "user4@email.com", "$pass", "user4.jpg", TO_BASE64('description of user 4'));
SQL);

			$pass = hash("md5", $password[4]);
			mysqli_query($conn, <<<SQL
			INSERT INTO users (name, email, password, picture, description) VALUES ("user5", "user5@email.com", "$pass", "user5.jpg", TO_BASE64('description of user 5'));
SQL);
...
<html lang="en">
<head>
	<title>XSS Training Lab by MyOPECS</title>
	<meta charset="utf-8">
...
<div class="container pt-4">
	<h1>
		Welcome to XSS Training Lab!<br />
		<small>Developed by MyOPECS.info (Malaysia Open Cyber Security Information)</small>
	</h1>
<?php
	if(defined("DONE")){
	?>
...
?>
	...
	<p>2. Create a database named <code>xss_lab</code>.</p>
